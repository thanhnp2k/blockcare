...
func (tx *Transaction) GetHash() *crypto.Hash {
	switch tx.Version {
	// case 1:
	// 	hashObj := crypto.Hash(crypto.Keccak256(tx.SerializeHeader()))
	// 	return &hashObj
	case 2:
		...
		hash := crypto.Hash(crypto.Keccak256(prefixHash[:], rctBaseHash[:], rctPrunableHash[:]))
		return &hash
	default:
		return nil
...
func (tx *Transaction) DeserializeHeader(buf []byte) (err error) {
	...
	// }
	log.Println(10, "transaction version %d\n", tx.Version)
	...
	for i := uint64(0); i < vin_length; i++ {
		...
		log.Println(10, "Processing i %d  vin_type %s  hex %x\n", i, TX_IN_NAME[vin_type], buf[:40])

		switch vin_type {
		case TXIN_GEN:
			log.Println(10, "Coinbase transaction\n")
	...
	for i := uint64(0); i < vout_length; i++ {
		...
		log.Println(10, "Vout Amount length %d  vout type %d   \n", amount, vout_type)
	...
	log.Println(8, "extra len %d  have %d \n", extra_length, len(buf))
	...
	*/
	...
	log.Println(8, "TX deserialized %+v\n", tx)
...
func (tx *Transaction) SerializeHeader() []byte {
	...
	//log.Println("Extra length %d while serializing\n ", len(tx.Extra))
