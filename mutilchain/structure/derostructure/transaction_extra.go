...
func (tx *Transaction) Parse_Extra() (result bool) {
	...
	for i := 0; ; i++ {
		...
		case TX_EXTRA_PADDING: // this is followed by 1 byte length, then length bytes of padding
			n, err = buf.Read(b)
			if err != nil {
				log.Println(1, "Extra padding length could not be parsed")
				return false
			}
			length_int = int(b[0])
			...
			if err != nil || n != int(length_int) {
				log.Println(1, "Extra padding could not be read  ")
				return false
			}
		...
		case TX_PUBLIC_KEY: // next 32 bytes are tx public key
			...
			n, err = buf.Read(pkey[:])
			if err != nil || n != 32 {
				log.Println(1, "Tx public key could not be parsed len=%d err=%s ", n, err)
				return false
			}
		...
		case TX_EXTRA_NONCE: // this is followed by 1 byte length, then length bytes of data
			n, err = buf.Read(b)
			if err != nil {
				log.Println(1, "Extra nonce length could not be parsed ")
				return false
			}
			...
			case 33: // unencrypted 32 byte  payment id
				if extra_nonce[0] == byte(TX_EXTRA_NONCE_PAYMENT_ID) {
					...
					log.Println(1, "Extra Nonce contains invalid payment id ")
					return false
				}
			...
			case 9: // encrypted 9 byte payment id
				if extra_nonce[0] == byte(TX_EXTRA_NONCE_ENCRYPTED_PAYMENT_ID) {
					...
					log.Println(1, "Extra Nonce contains invalid encrypted payment id ")
					return false
				}
...
func (tx *Transaction) Serialize_Extra() []byte {
	...
	if _, ok := tx.Extra_map[TX_PUBLIC_KEY]; ok {
		...
		log.Println(1, "TX does not contain a Public Key, not possible, the transaction will be rejected")
	...
	if _, ok := tx.PaymentID_map[TX_EXTRA_NONCE_PAYMENT_ID]; ok {
		...
		log.Println(1, "unencrypted payment id size mismatch expected = %d actual %d", 32, len(data_bytes))
	}
	...
	if _, ok := tx.PaymentID_map[TX_EXTRA_NONCE_ENCRYPTED_PAYMENT_ID]; ok {
		...
		log.Println(1, "unencrypted payment id size mismatch expected = %d actual %d", 8, len(data_bytes))
	}
	...
	if _, ok := tx.Extra_map[TX_EXTRA_NONCE]; ok {
		...
		if len(data_bytes) > 255 {
			log.Println(1, "TX extra none is spilling, trimming the nonce to 254 bytes")
